      ...
      // WHY: NApp.setup() is async — wait for the chat panel to exist.
      const layout = document.querySelector('native-dashboard-spa');
      await customElements.whenDefined('native-dashboard-spa');
      const chatPanel = await Promise.race([
        new Promise(resolve => {
          const check = () => {
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error('Chat panel not found after 5s')), 5000)),
      ]);
      ...
      for (const [id, msg] of Object.entries(PRESETS)) {
        document.getElementById(id)?.addEventListener('native:press', () => {
          // Clear all surfaces
