...
describe('n-app-panel', () => {
  it('is registered as a custom element', () => {
    expect(customElements.get('n-app-panel')).toBeDefined();
  });

  // ── Main mode (default) ──

  it('main mode: renders without aside attribute', () => {
    const el = document.createElement('n-app-panel');
    document.body.appendChild(el);
    expect(el.hasAttribute('aside')).toBe(false);
  });

  it('main mode: open defaults to false', () => {
    const el = document.createElement('n-app-panel') as any;
    document.body.appendChild(el);
