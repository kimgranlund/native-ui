...
import '../../../../src/components/toolbar/toolbar.ts';
import '../../../../src/icons/icon.ts';

import '../../../../src/icons/phosphor/bell.ts';
import '../../../../src/icons/phosphor/info.ts';
import '../../../../src/icons/phosphor/check-circle.ts';
import '../../../../src/icons/phosphor/warning.ts';
import '../../../../src/icons/phosphor/x-circle.ts';
import '../../../../src/icons/phosphor/x.ts';
import '../../../../src/icons/phosphor/trash.ts';
import '../../../../src/icons/phosphor/users.ts';
import '../../../../src/icons/phosphor/pulse.ts';
import '../../../../src/icons/phosphor/gear.ts';
import '../../../../src/icons/phosphor/terminal.ts';
import '../../../../src/icons/phosphor/chart-bar.ts';
import '../../../../src/icons/phosphor/rocket.ts';
import '../../../../src/icons/phosphor/shield.ts';
import '../../../../src/icons/phosphor/database.ts';
import '../../../../src/icons/phosphor/cloud.ts';
import '../../../../src/icons/phosphor/git-branch.ts';
import '../../../../src/icons/phosphor/lightning.ts';
import '../../../../src/icons/phosphor/package.ts';
import '../../../../src/icons/phosphor/broadcast.ts';
...
function pick(a: any[]) { return a[Math.floor(Math.random() * a.length)]; }
function timestamp() {
  const d = new Date();
...
let nextId = 0;
const logEntries = signal<{ id: number; tag: string; msg: string; ts: string }[]>([]);
function ctxLog(tag: string, msg: string) {
...
const ProviderBase = ContextProvider(NativeElement);
const ConsumerBase = ContextConsumer(NativeElement);
...
class NotifProvider extends ProviderBase {
  setup() {
    super.setup();
    const items = signal<{ id: number; title: string; msg: string; type: string }[]>([]);
    const count = computed(() => items.value.length);
    const stats = signal({ info: 0, success: 0, warning: 0, danger: 0, total: 0, dismissed: 0 });

    const push = (title: string, msg: string, type = 'info') => {
      items.value = [{ id: ++nextId, title, msg, type }, ...items.value];
      stats.value = { ...stats.value, [type]: (stats.value as any)[type] + 1, total: stats.value.total + 1 };
...
class AppToolbar extends ConsumerBase {
  setup() {
    ...
    this.innerHTML = `
      <n-toolbar>
        <div class="avatar-wrap">
          <n-avatar name="You" size="sm" intent="accent">Y</n-avatar>
          <n-badge intent="success" floating dot></n-badge>
        </div>
        ...
        <n-button variant="ghost" size="sm" intent="info" data-action="push">
          <n-icon name="lightning" slot="leading"></n-icon> Generate Event
        </n-button>
      </n-toolbar>
    `;

    this.requestContext('session', (ctx: any) => {
    ...
    this.requestContext('notifications', (_ctx: any) => {
      ctxLog('notif', 'Toolbar subscribed to notifications');
    });

    this.addEventListener('click', (e: Event) => {
      const btn = (e.target as Element).closest('[data-action="push"]');
      if (!btn) return;
      const ctx = this.getContext('notifications') as any;
      if (!ctx) return;
      const type = pick(['info', 'success', 'warning', 'danger']);
      const tmpl = pick(NOTIF_POOL[type as keyof typeof NOTIF_POOL]);
        ...
        this.innerHTML = TEAM.map(m => {
          ...
          return `
            <div class="row" style="display:flex;align-items:center;gap:0.5rem;">
              <div class="avatar-wrap">
                <n-avatar name="${name}" size="sm" intent="${m.intent}">${name.split(' ').map((p: string) => p[0]).join('').toUpperCase().slice(0,2)}</n-avatar>
                <n-badge intent="${m.online ? 'success' : 'neutral'}" floating dot></n-badge>
              </div>
              <div>
...
class NotifPanelActions extends ConsumerBase {
  setup() {
    super.setup();
    this.requestContext('notifications', (ctx: any) => {
      this.addEffect(() => {
        const c = ctx.count.value;
        this.innerHTML = `
          ${c > 0 ? `<n-badge intent="danger" size="xs">${c}</n-badge>` : ''}
          <n-button variant="ghost" size="xs" data-action="clear" ${c === 0 ? 'disabled' : ''}>
    ...
    this.addEventListener('click', (e: Event) => {
      if ((e.target as Element).closest('[data-action="clear"]')) {
        const ctx = this.getContext('notifications') as any;
        if (ctx) ctx.clear();
      }
    });
  ...
  setup() {
    ...
    this.requestContext('notifications', (ctx: any) => {
      ...
      this.addEffect(() => {
        ...
        if (list.length === 0) {
          this.innerHTML = `
            <n-stack align="center" gap="2" style="padding: 2rem 0; color: var(--n-ink-muted-neutral); font-size: 0.8125rem;">
              <n-icon name="bell" size="28"></n-icon>
              <span>All clear — no notifications</span>
            </n-stack>`;
          return;
        }
        this.innerHTML = list.map((n: any) => `
          <div class="notif-row" style="display:flex;align-items:flex-start;gap:0.5rem;padding:0.5rem 0;border-bottom:1px solid var(--n-border-muted-neutral);">
            <div class="activity-dot ${n.type}">
              <n-icon name="${INTENT_ICONS[n.type as keyof typeof INTENT_ICONS]}" size="14"></n-icon>
            </div>
            <div class="notif-text" style="flex:1;min-width:0;">
              <strong>${n.title}</strong>
              <p>${n.msg}</p>
            </div>
            <n-button variant="ghost" size="xs" data-dismiss="${n.id}">
    ...
    this.addEventListener('click', (e: Event) => {
      const btn = (e.target as Element).closest('[data-dismiss]');
      if (!btn) return;
      const ctx = this.getContext('notifications') as any;
      if (ctx) ctx.dismiss(Number((btn as HTMLElement).dataset.dismiss));
    });
  }
}
...
class ActivityFeed extends ConsumerBase {
  setup() {
    ...
    this.addEffect(() => {
      ...
      if (list.length === 0) {
        this.innerHTML = '<div style="text-align:center;padding:2rem;color:var(--n-ink-muted-neutral);font-size:0.8125rem;">No activity yet</div>';
        return;
      }
      this.innerHTML = list.map(a => `
        <div class="activity-row" style="display:flex;align-items:flex-start;gap:0.5rem;padding:0.5rem 0;border-bottom:1px solid var(--n-border-muted-neutral);">
          <div class="activity-dot ${a.intent}">
            <n-icon name="${a.icon}" size="14"></n-icon>
          </div>
  ...
  setup() {
    ...
    this.requestContext('session', (ctx: any) => {
      ...
      this.innerHTML = PREFS.map(p => `
        <div style="display:flex;align-items:center;justify-content:space-between;gap:0.75rem;padding:0.5rem 0;border-bottom:1px solid var(--n-border-muted-neutral);">
          <div>
            <div class="pref-label">${p.label}</div>
            <div class="pref-desc">${p.desc}</div>
          </div>
          <n-switch size="sm" data-pref="${p.key}" ${ctx.prefs[p.key].value ? 'checked' : ''}></n-switch>
        </div>
      `).join('');
    });

    this.addEventListener('native:change', (e: Event) => {
      const sw = (e.target as Element).closest('n-switch') as HTMLElement | null;
      if (!sw) return;
      const key = (sw as any).dataset.pref;
      const ctx = this.getContext('session') as any;
      if (key && ctx?.prefs[key]) {
        ctx.prefs[key].value = (sw as any).checked;
        ctxLog('pref', `${key} → ${(sw as any).checked ? 'on' : 'off'}`);
      }
    });
  }
}
      ...
      this.addEffect(() => {
        ...
        this.innerHTML = `
          <n-stack gap="3">
            <n-grid cols="4" gap="2">
              <n-stack gap="1">
                <div class="stat-value" style="color: var(--n-ink-info);">${s.info}</div>
                <div class="stat-label">Info</div>
              </n-stack>
              <n-stack gap="1">
                <div class="stat-value" style="color: var(--n-ink-success);">${s.success}</div>
                <div class="stat-label">Success</div>
              </n-stack>
              <n-stack gap="1">
                <div class="stat-value" style="color: var(--n-ink-warning);">${s.warning}</div>
                <div class="stat-label">Warning</div>
              </n-stack>
              <n-stack gap="1">
                <div class="stat-value" style="color: var(--n-ink-danger);">${s.danger}</div>
                <div class="stat-label">Danger</div>
              </n-stack>
            </n-grid>
            <div style="display:flex;justify-content:center;gap:1.5rem;font-size:0.75rem;color:var(--n-ink-muted-neutral);">
              <span><strong style="color:var(--n-ink-neutral);">${s.total}</strong> total</span>
              <span><strong style="color:var(--n-ink-neutral);">${s.dismissed}</strong> dismissed</span>
              <span><strong style="color:var(--n-ink-neutral);">${ctx.count.value}</strong> pending</span>
            </div>
          </n-stack>`;
      });
    });
...
class CtxLog extends NativeElement {
  setup() {
    super.setup();
    this.addEffect(() => {
      const entries = logEntries.value;
      if (entries.length === 0) {
        this.innerHTML = '<div class="ctx-log" style="font-style:italic;color:var(--n-ink-muted-neutral);">Waiting for events...</div>';
        return;
      }
