import { signal, UIElement, PopoverController } from '@nonoun/native-ui';
 ...
 */
export class NuiSidebarGroup extends UIElement {
  ...
  setup(): void {
    ...
    // Move <nui-sidebar-group-header> content into <summary>
    const header = this.querySelector(':scope > nui-sidebar-group-header');
    if (header) {
    ...
    // the sidebar edge — same pattern as nui-sidebar-item menus.
    this.#popover.wirePopover(this, flyout);

    // WHY: ui-option click → ui-select bubbles through listbox → group → nav.
    ...
    // from leaking to nui-sidebar-nav (which would cause duplicate ui-change events).
  ...
  #syncIndicator(): void {
    ...
    const items = this.#details.querySelectorAll<HTMLElement>(':scope > nui-sidebar-nav-item');
    let selectedIndex = -1;
  ...
  openFlyout(): void {
    ...
    // NOT nui-sidebar-nav-item — the collapsed sidebar rule hides all nui-sidebar-nav-item
    // descendants, and that selector matches even in the top-layer popover.
    this.#flyout.innerHTML = '';
    const sources = this.#details.querySelectorAll<HTMLElement>(':scope > nui-sidebar-nav-item');
    let first = true;
  ...
  #onFlyoutSelect = (): void => {
    ...
    // bubbling to nui-sidebar-nav before we clear the flyout DOM. Synchronous
  ...
  #onFlyoutChange = (e: Event): void => {
    ...
    // Stop it here so nui-sidebar-nav doesn't get a duplicate ui-change
    // (nui-sidebar-nav dispatches its own ui-change from the ui-select handler).
    e.stopPropagation();
  };
}
