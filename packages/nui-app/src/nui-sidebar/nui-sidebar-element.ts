...
import type { NuiSidebarGroup } from './nui-sidebar-group-element.ts';
...
export class NuiSidebar extends UIElement {
  ...
  #activeGroup: NuiSidebarGroup | null = null;
  ...
  setup(): void {
    ...
    // WHY: queueMicrotask — child nui-sidebar-group elements exist in the DOM but haven't
  ...
  #syncFlyoutMode(): void {
    ...
    const groups = this.querySelectorAll<NuiSidebarGroup>('nui-sidebar-group');
  ...
  #onGroupDismiss = (e: Event): void => {
    const group = (e.target as HTMLElement)?.closest?.('nui-sidebar-group') as NuiSidebarGroup | null;
  ...
  #observeOverlays(aside: HTMLElement): void {
    const header = aside.querySelector('nui-sidebar-header') as HTMLElement | null;
    const footer = aside.querySelector('nui-sidebar-footer') as HTMLElement | null;
    const content = aside.querySelector('nui-sidebar-content') as HTMLElement | null;
    if (!content) return;
