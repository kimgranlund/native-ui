...
**n-panel** -- Section surface with `container-type: inline-size`. Attributes: `padding`, `dividers`, `bordered`, `scrollable`, `show-scrollbar`. Panels don't scroll by default — add `[scrollable]` for opt-in `overflow-y: auto` when bounded by height. Aside mode: `[aside]` enables collapsible side panel (360px default, 280-480px range) with animated open/close and resize handle. `[aside][open]` shows the panel.
