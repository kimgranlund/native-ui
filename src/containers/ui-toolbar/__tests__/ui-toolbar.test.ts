...
function create(attrs: Record<string, string> = {}, children: string[] = []): HTMLElement {
  ...
  for (const html of children) {
    const child = document.createElement('ui-button');
...
function getButtons(toolbar: HTMLElement): HTMLElement[] {
  return [...toolbar.querySelectorAll<HTMLElement>('ui-button:not([disabled])')];
}
...
describe('ui-toolbar', () => {
  it('is registered as a custom element', () => {
    expect(customElements.get('ui-toolbar')).toBeDefined();
  });

  it('creates an instance via document.createElement', () => {
    const el = create();
    expect(el).toBeInstanceOf(HTMLElement);
    expect(el.tagName.toLowerCase()).toBe('ui-toolbar');
  });
  ...
  describe('ARIA', () => {
    it('sets role=toolbar via ElementInternals', () => {
      ...
      const Ctor = customElements.get('ui-toolbar')!;
  ...
  describe('roving focus', () => {
    ...
    it('skips disabled buttons', () => {
      const el = document.createElement('ui-toolbar');
      const a = document.createElement('ui-button');
      a.textContent = 'A';
      const b = document.createElement('ui-button');
      b.textContent = 'B';
      b.setAttribute('disabled', '');
      const c = document.createElement('ui-button');
      c.textContent = 'C';
