...
export class UICombobox extends FormAssociable(UIElement) {
  ...
  #parseOptions(json: string): ComboboxOption[] {
    return parseDataOptions<ComboboxOption>(json, 'ui-combobox');
  }

  async #fetchOptions(url: string): Promise<void> {
    this.#fetchController = await fetchDataOptions<ComboboxOption>(
      url, this.#fetchController, this.#options, 'ui-combobox',
    );
  }
  ...
  setup(): void {
    ...
    const input = this.querySelector<HTMLElement>(':scope > ui-input');
    this.#input = input;
    const listbox = this.querySelector<HTMLElement>(':scope > ui-listbox[popover]');
    this.#listbox = listbox;

    if (__DEV__ && !this.#dataMode) {
      if (!input) console.warn('[ui-combobox] Manual mode requires a <ui-input> child. None found.');
      if (!listbox) console.warn('[ui-combobox] Manual mode requires a <ui-listbox popover> child. None found.');
    }
    ...
    this.deferChildren(() => {
      ...
      if (!this.#dataMode) {
        const optionEls = listbox?.querySelectorAll('ui-option') ?? [];
      ...
      this.addEffect(() => {
        ...
        const allOptions = listbox?.querySelectorAll('ui-option') ?? [];
      ...
      this.addEffect(() => {
        ...
        const visibleOptions = [...(listbox?.querySelectorAll<HTMLElement>('ui-option:not([hidden]):not([disabled])') ?? [])];
    ...
    this.addEventListener('ui-input', this.#onInput);

    // WHY: Stop child ui-change events (from ui-listbox and ui-input blur) from leaking out —
    // the combobox fires its own canonical ui-change on option selection.
    ...
    // Event: option selection → store.select + public ui-change
  ...
  teardown(): void {
    ...
    this.removeEventListener('ui-input', this.#onInput);
