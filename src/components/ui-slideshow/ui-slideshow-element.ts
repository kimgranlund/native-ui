 ...
 * @fires ui-slide-change - Fired when the active slide changes with `{ index, slide }` detail
 */
export class UISlideshow extends UIElement {
  ...
  setup(): void {
    ...
    while (this.firstChild) {
      ...
      if (child instanceof HTMLElement && child.tagName.toLowerCase() === 'ui-slide') {
        slides.push(child);
      }
    }
    ...
    if (__DEV__ && slides.length === 0) {
      console.warn('[ui-slideshow] No <ui-slide> children found.');
    }
  ...
  #applyPerView(val: string | null): void {
    ...
    this.style.setProperty('--_per-view', String(n));
  }
        ...
        for (const entry of entries) {
          if (entry.isIntersecting && entry.intersectionRatio >= 0.5) {
            ...
            if (idx !== -1 && idx !== this.#activeIndex.value) {
              ...
              this.dispatchEvent(new CustomEvent('ui-slide-change', {
                bubbles: true,
                composed: true,
