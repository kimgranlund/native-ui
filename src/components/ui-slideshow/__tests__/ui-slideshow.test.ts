...
function create(opts: { slides?: number; attrs?: Record<string, string> } = {}): HTMLElement {
  const el = document.createElement('ui-slideshow');
  for (const [k, v] of Object.entries(opts.attrs ?? {})) {
    el.setAttribute(k, v);
  }
  const count = opts.slides ?? 3;
  for (let i = 0; i < count; i++) {
    const slide = document.createElement('ui-slide');
...
describe('ui-slideshow', () => {
  it('is registered as a custom element (both ui-slideshow and ui-slide)', () => {
    expect(customElements.get('ui-slideshow')).toBeDefined();
    expect(customElements.get('ui-slide')).toBeDefined();
  });
  ...
  it('moves slides into track', () => {
    ...
    const slides = track.querySelectorAll('ui-slide');
  ...
  it('per-view attribute sets --_per-view CSS variable', () => {
    const el = create({ attrs: { 'per-view': '3' } });
    expect(el.style.getPropertyValue('--_per-view')).toBe('3');
  });
  ...
  it('ui-slide sets tabindex="-1" in setup', () => {
    const el = create({ slides: 1 });
    const track = el.querySelector('[part="track"]')!;
    const slide = track.querySelector('ui-slide')!;
  ...
  it('per-view defaults to 1 when not set', () => {
    const el = create();
    expect(el.style.getPropertyValue('--_per-view')).toBe('1');
  });
