...
function create(opts: { multiple?: boolean } = {}): HTMLElement {
  const el = document.createElement('n-listbox');
  if (opts.multiple) el.setAttribute('multiple', '');

  const opt1 = document.createElement('n-option');
  opt1.setAttribute('value', 'a');
  opt1.textContent = 'Alpha';
  el.appendChild(opt1);

  const opt2 = document.createElement('n-option');
  opt2.setAttribute('value', 'b');
  opt2.textContent = 'Beta';
  el.appendChild(opt2);

  const opt3 = document.createElement('n-option');
...
describe('n-listbox element', () => {
  it('is registered as a custom element', () => {
    expect(customElements.get('n-listbox')).toBeDefined();
  });

  it('n-option is registered as a custom element', () => {
    expect(customElements.get('n-option')).toBeDefined();
  });
  ...
  it('option click dispatches native:select event with value and label', () => {
    const el = create();
    const handler = vi.fn();
    el.addEventListener('native:select', handler);

    const opt = el.querySelector('n-option[value="a"]') as HTMLElement;
    opt.click();
  ...
  it('disabled option does not dispatch native:select', () => {
    const el = create();
    const handler = vi.fn();
    el.addEventListener('native:select', handler);

    const opt = el.querySelector('n-option[value="c"]') as HTMLElement;
    opt.click();
...
describe('n-option element', () => {
  it('value property reflects to attribute', () => {
    const el = create();
    const opt = el.querySelector('n-option[value="a"]') as any;
    opt.value = 'x';
...
describe('n-listbox selection', () => {
  it('value updates on option selection via native:select event', () => {
    const el = create();
    const opt = el.querySelector('n-option[value="b"]') as HTMLElement;
    opt.click();
  ...
  it('does not dispatch native:change when listbox is disabled', () => {
    const el = create();
    ...
    const opt = el.querySelector('n-option[value="a"]') as HTMLElement;
    // Dispatch native:select directly from option (simulating click bypass)
  ...
  it('multiple mode: can select multiple values', () => {
    ...
    // Select first option
    const opt1 = el.querySelector('n-option[value="a"]') as HTMLElement;
    opt1.click();
    ...
    const opt2 = el.querySelector('n-option[value="b"]') as HTMLElement;
    opt2.click();
    expect((el as any).value).toBe('b');
...
describe('n-listbox ARIA', () => {
  it('has appropriate role via internals', () => {
    ...
    expect(el.tagName.toLowerCase()).toBe('n-listbox');
