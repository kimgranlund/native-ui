...
function create(attrs: Record<string, string> = {}, content = 'Dialog content'): HTMLElement {
  const el = document.createElement('n-dialog');
...
describe('n-dialog', () => {
  it('is registered as a custom element', () => {
    expect(customElements.get('n-dialog')).toBeDefined();
  });
  ...
  it('creates a <dialog> element on setup and moves children into it', () => {
    ...
    // The direct child of n-dialog should be the <dialog>, not the text node
  ...
  it('native:dismiss event closes the dialog', () => {
    const el = create();
    ...
    el.dispatchEvent(new Event('native:dismiss', { bubbles: true }));

    expect((el as any).open).toBe(false);
    expect(closeFired).toHaveBeenCalledTimes(1);
  });

  it('native:dismiss does NOT close when no-close-on-escape is present', () => {
    const el = create({ 'no-close-on-escape': '' });
  ...
  it('opt-out attributes are read at event time (not in observedAttributes)', () => {
    ...
    const Ctor = customElements.get('n-dialog')!;
