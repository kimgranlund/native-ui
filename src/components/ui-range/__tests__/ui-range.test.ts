...
function create(attrs: Record<string, string> = {}): HTMLElement {
  const el = document.createElement('ui-range');
...
describe('ui-range', () => {
  ...
  it('is registered as a custom element', () => {
    expect(customElements.get('ui-range')).toBeDefined();
  });

  // ── Setup ──

  it('creates a .ui-range-thumb child element on setup', () => {
    const el = create();
    const thumb = el.querySelector('.ui-range-thumb');
  ...
  it('keyboard dispatches ui-input when value changes', () => {
    const el = create({ value: '50' });
    const handler = vi.fn();
  ...
  it('keyboard dispatches ui-change when value changes', () => {
    const el = create({ value: '50' });
    const handler = vi.fn();
  ...
  it('keyboard does not dispatch events when value does not change (already at boundary)', () => {
    ...
    el.addEventListener('ui-input', inputHandler);
    el.addEventListener('ui-change', changeHandler);
