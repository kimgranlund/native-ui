...
export class ColumnResizeController {
  ...
  #onPointerDown = (e: PointerEvent): void => {
    ...
    const header = handle.closest('n-table-header');
    if (!header) return;

    const row = header.closest('n-table-row');
    if (!row) return;
    const headers = [...row.querySelectorAll('n-table-header')];
    ...
    this.table.dispatchEvent(new CustomEvent('native:table-resize-start', {
      bubbles: true,
      composed: true,
  ...
  #onPointerMove = (e: PointerEvent): void => {
    ...
    this.table.dispatchEvent(new CustomEvent('native:table-resize', {
      bubbles: true,
      composed: true,
  ...
  #onPointerUp = (_e: PointerEvent): void => {
    ...
    this.table.dispatchEvent(new CustomEvent('native:table-resize-end', {
      bubbles: true,
      composed: true,
      detail: {
