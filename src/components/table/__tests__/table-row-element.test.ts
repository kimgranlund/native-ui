...
describe('n-table-row', () => {
  ...
  it('dispatches native:row-select on click when table is selectable', () => {
    const row = createRow({ value: 'row-1' }, true);
    const handler = vi.fn();
    row.addEventListener('native:row-select', handler);
  ...
  it('does not dispatch native:row-select when table is not selectable', () => {
  ...
  it('native:row-select event bubbles and is composed', () => {
    const row = createRow({ value: 'x' }, true);
    let event: CustomEvent | null = null;
    row.addEventListener('native:row-select', (e) => { event = e as CustomEvent; });
