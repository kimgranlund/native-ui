...
function createCommand(opts: { withInput?: boolean; withEmpty?: boolean } = {}): HTMLElement {
  const cmd = document.createElement('ui-command');

  if (opts.withInput !== false) {
    const input = document.createElement('ui-command-input');
  ...
  const list = document.createElement('ui-command-list');

  const item1 = document.createElement('ui-command-item');
  item1.setAttribute('value', 'copy');
  item1.textContent = 'Copy';
  list.appendChild(item1);

  const item2 = document.createElement('ui-command-item');
  item2.setAttribute('value', 'paste');
  item2.textContent = 'Paste';
  list.appendChild(item2);

  const item3 = document.createElement('ui-command-item');
  ...
  if (opts.withEmpty) {
    const empty = document.createElement('ui-command-empty');
...
function createGroupedCommand(): HTMLElement {
  ...
  // Group 1: Edit
  const group1 = document.createElement('ui-command-group');
  ...
  // Group 2: View
  const group2 = document.createElement('ui-command-group');
...
describe('ui-command registration', () => {
  it('is registered as a custom element', () => {
    expect(customElements.get('ui-command')).toBeDefined();
  });

  it('registers all sub-elements', () => {
    expect(customElements.get('ui-command-input')).toBeDefined();
...
describe('ui-command-item', () => {
  it('value property reflects attribute', () => {
    const cmd = createCommand();
    const item = cmd.querySelector('ui-command-item[value="copy"]') as any;
    expect(item.value).toBe('copy');
  });

  it('value property setter reflects to attribute', () => {
    const cmd = createCommand();
    const item = cmd.querySelector('ui-command-item') as any;
  ...
  it('dispatches ui-select on click', () => {
    const cmd = createCommand();
    const item = cmd.querySelector('ui-command-item[value="copy"]')!;
    const handler = vi.fn();
    cmd.addEventListener('ui-select', handler);
  ...
  it('disabled item does not dispatch ui-select on click', () => {
    const cmd = createCommand();
    const item = cmd.querySelector('ui-command-item[value="copy"]') as any;
    item.disabled = true;
    const handler = vi.fn();
  ...
  it('keywords attribute updates property', () => {
    const cmd = createCommand();
    const item = cmd.querySelector('ui-command-item')!;
    item.setAttribute('keywords', 'baz qux');
    expect((item as any).keywords).toBe('baz qux');
  });
});

// ── Command dispatches ui-change on ui-select ──

describe('ui-command event re-dispatch', () => {
  it('dispatches ui-change when item emits ui-select', () => {
    const cmd = createCommand();
    const handler = vi.fn();
    cmd.addEventListener('ui-change', handler);
...
describe('ui-command filtering', () => {
  it('typing in input dispatches ui-input and updates store query', async () => {
    const cmd = createCommand();
    const input = cmd.querySelector('ui-command-input input') as HTMLInputElement;
  ...
  it('filtering hides non-matching items', async () => {
    ...
    const items = cmd.querySelectorAll('ui-command-item');
...
describe('ui-command keyboard navigation', () => {
  ...
  it('Escape dispatches ui-dismiss', () => {
    const cmd = createCommand();
    const handler = vi.fn();
    cmd.addEventListener('ui-dismiss', handler);
...
describe('ui-command-group', () => {
  it('groups items with heading', () => {
    ...
    const groups = cmd.querySelectorAll('ui-command-group');
...
describe('ui-command empty state', () => {
  it('shows empty element when no items match', async () => {
    ...
    await Promise.resolve();
    const empty = cmd.querySelector('ui-command-empty')!;
...
describe('ui-command-input', () => {
  ...
  it('clear() resets the input value and dispatches ui-input', async () => {
    ...
    const handler = vi.fn();
    cmd.addEventListener('ui-input', handler);
  ...
  it('dispatches ui-input on native input event', async () => {
    const cmd = createCommand();
    await Promise.resolve();
    const handler = vi.fn();
    cmd.addEventListener('ui-input', handler);

    const nativeInput = cmd.querySelector('ui-command-input input') as HTMLInputElement;
  ...
  it('inputElement is null before setup', () => {
    ...
    const cmdInput = document.createElement('ui-command-input');
