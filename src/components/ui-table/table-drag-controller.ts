import { DragController } from '../../traits/drag-controller.ts';
 ...
 * - Auto DOM reorder via ui-table-reorder event
 */
export class TableDragController {
  ...
  constructor(tableBody: HTMLElement, table: HTMLElement) {
    ...
    tableBody.addEventListener('ui-drag-start', this.#onDragStart as EventListener);
    tableBody.addEventListener('ui-drop', this.#onDrop as EventListener);
  ...
  #onDragStart = (_e: Event): void => {
    ...
    // WHY: The ghost is a clone of ui-table-row which uses subgrid.
    ...
    // Ghost is appended to body BEFORE ui-drag-start fires, so it's in DOM.
  ...
  #onDrop = (e: Event): void => {
    ...
    const event = new CustomEvent('ui-table-reorder', {
      bubbles: true,
      composed: true,
      cancelable: true,
      detail: {
  ...
  destroy(): void {
    this.#body.removeEventListener('ui-drag-start', this.#onDragStart as EventListener);
    this.#body.removeEventListener('ui-drop', this.#onDrop as EventListener);
