...
function createTree(): HTMLElement {
  const tree = document.createElement('n-tree');

  const item1 = document.createElement('n-tree-item');
  ...
  const item2 = document.createElement('n-tree-item');
...
describe('n-tree', () => {
  it('is registered as a custom element (both n-tree and n-tree-item)', () => {
    expect(customElements.get('n-tree')).toBeDefined();
  ...
  it('tree sets aria-label="Tree" by default', async () => {
    ...
    expect(tree.tagName.toLowerCase()).toBe('n-tree');
  });
  ...
  it('click on label selects item (adds [selected])', async () => {
    const tree = createTree();
    await tick();
    const item1 = tree.querySelector('n-tree-item')!;
  ...
  it('tree-item sets --n-tree-depth CSS property on nested labels (depth > 0)', async () => {
    const tree = createNestedTree();
    await tick();

    // The child item is nested inside the parent -> depth = 1
    const childrenWrapper = tree.querySelector('.n-tree-children');
    expect(childrenWrapper).not.toBeNull();

    const childItem = childrenWrapper!.querySelector('n-tree-item')!;
    const childLabel = childItem.querySelector('[slot="label"]') as HTMLElement;
    expect(childLabel.style.getPropertyValue('--n-tree-depth')).toBe('1');
  });

  it('wraps child tree-items in a .n-tree-children container', async () => {
    const tree = createNestedTree();
    await tick();
    const parentItem = tree.querySelector(':scope > n-tree-item')!;
    const wrapper = parentItem.querySelector('.n-tree-children');
    expect(wrapper).not.toBeNull();
    expect(wrapper!.querySelectorAll('n-tree-item').length).toBe(1);
  });
  ...
  it('flat items do not get [expandable] attribute', async () => {
    const tree = createTree();
    await tick();
    const items = tree.querySelectorAll('n-tree-item');
