 ...
 * @fires native:change - Fired on single date selection with `{ value }` detail
 * @fires native:range-select - Fired on range commit with `{ start, end }` detail
 */
export class NCalendar extends FormAssociable(NativeElement) {
  ...
  #selectDate(iso: string): void {
    if (this.range) {
      ...
      if (this.#rangePhase === 'idle') {
        ...
        this.dispatchEvent(new CustomEvent('native:range-select', {
          bubbles: true,
          composed: true,
