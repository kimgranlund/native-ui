...
function create(attrs: Record<string, string> = {}): HTMLElement {
  const el = document.createElement('ui-input-otp');
...
describe('ui-input-otp', () => {
  it('is registered as a custom element', () => {
    expect(customElements.get('ui-input-otp')).toBeDefined();
  });

  it('stamps 6 cells by default', () => {
    const el = create();
    const cells = el.querySelectorAll('.ui-otp-cell');
  ...
  it('dispatches ui-input on cell input', () => {
    const el = create();
    const handler = vi.fn();
  ...
  it('dispatches ui-change when all cells filled', () => {
    ...
    // Fill first two — no ui-change yet
    cells[0].textContent = '1';
    cells[0].dispatchEvent(new Event('input', { bubbles: true }));
    ...
    // Fill the last cell — ui-change fires
