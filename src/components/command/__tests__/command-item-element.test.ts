...
function create(attrs: Record<string, string> = {}, text = 'Item'): HTMLElement {
  const el = document.createElement('n-command-item');
...
describe('n-command-item', () => {
  ...
  it('native:select event bubbles and is composed', () => {
    const el = create({ value: 'x' });
    let event: CustomEvent | null = null;
    el.addEventListener('native:select', (e) => { event = e as CustomEvent; });
