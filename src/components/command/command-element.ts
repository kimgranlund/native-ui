 ...
 * @fires native:change - Fired when an item is selected with `{ value, label }` detail
 * @fires native:dismiss - Fired on Escape key press
 */
export class NCommand extends NativeElement {
  ...
  setup(): void {
    ...
    this.addEventListener('native:input', this.#onInput as EventListener);
    this.addEventListener('native:select', this.#onSelect as EventListener);
    this.addEventListener('keydown', this.#onKeyDown);

    this.deferChildren(() => {
      if (__DEV__) {
        if (!this.querySelector('n-command-item')) console.warn('[n-command] No <n-command-item> children found.');
      }

      // WHY: Filter items when query changes
      this.addEffect(() => {
        const q = this.#store.query.value.toLowerCase().trim();
        const items = this.querySelectorAll<NCommandItem & HTMLElement>('n-command-item');
      ...
      this.addEffect(() => {
        ...
        const list = this.querySelector('n-command-list');
      ...
      this.addEffect(() => {
        ...
        const items = this.#getVisibleItems();
        const empty = this.querySelector('n-command-empty');
        if (empty) {
  ...
  teardown(): void {
    this.removeEventListener('native:input', this.#onInput as EventListener);
    this.removeEventListener('native:select', this.#onSelect as EventListener);
    this.removeEventListener('keydown', this.#onKeyDown);
    super.teardown();
  }

  #getVisibleItems(): (NCommandItem & HTMLElement)[] {
    return [...this.querySelectorAll<NCommandItem & HTMLElement>(
      'n-command-item:not([hidden]):not([disabled])',
    )];
  }
  ...
  #onKeyDown = (e: KeyboardEvent): void => {
    ...
    switch (e.key) {
      ...
      case 'Escape':
        this.dispatchEvent(new CustomEvent('native:dismiss', {
          bubbles: true,
          composed: true,
        }));
        break;
