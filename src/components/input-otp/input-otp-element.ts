 ...
 * @fires native:input - Fired on each cell change with `{ value }` detail
 * @fires native:change - Fired when all cells are filled with `{ value }` detail
 */
export class NInputOtp extends FormAssociable(NativeElement) {
