...
export class UITreeItem extends UIElement {
  ...
  #organizeChildren(): void {
    ...
    const childItems = this.querySelectorAll(':scope > ui-tree-item');
    ...
    while (parent) {
      if (parent.tagName === 'UI-TREE-ITEM') depth++;
      else if (parent.classList?.contains('ui-tree-children')) { /* skip wrapper */ }
      else break;
    ...
    if (depth > 0) {
      ...
      if (label instanceof HTMLElement) {
        label.style.setProperty('--_tree-depth', String(depth));
      }
    }
    ...
    switch (e.key) {
      case 'ArrowRight':
        if (this.hasAttribute('expandable') && !this.hasAttribute('expanded')) {
          ...
          // Focus first child
          const firstChild = this.querySelector(':scope > .ui-tree-children > ui-tree-item > [slot="label"]') as HTMLElement;
      ...
      case 'ArrowLeft':
        if (this.hasAttribute('expanded')) {
          ...
          const parentItem = this.parentElement?.closest('ui-tree-item');
