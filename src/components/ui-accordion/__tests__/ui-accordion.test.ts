...
function createAccordion(opts: { multiple?: boolean; items: { heading: string; content: string; open?: boolean }[] }): HTMLElement {
  ...
  for (const item of opts.items) {
    const el = document.createElement('ui-accordion-item');
...
describe('ui-accordion', () => {
  ...
  it('stamps details/summary inside accordion-item', () => {
    ...
    const item = acc.querySelector('ui-accordion-item')!;
  ...
  it('respects initial open attribute', () => {
    ...
    const item = acc.querySelector('ui-accordion-item')! as any;
  ...
  it('single mode: opening one item closes others', () => {
    ...
    const items = acc.querySelectorAll('ui-accordion-item') as NodeListOf<any>;
