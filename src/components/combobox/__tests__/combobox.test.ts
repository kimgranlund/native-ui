...
function create(opts: { value?: string; disabled?: boolean } = {}): HTMLElement {
  const el = document.createElement('n-combobox');
...
describe('n-combobox', () => {
  it('is registered as a custom element', () => {
    expect(customElements.get('n-combobox')).toBeDefined();
  });
  ...
  it('input has role="combobox", aria-autocomplete="list", and aria-expanded="false"', () => {
    const el = create();
    const input = el.querySelector('n-input')!;
  ...
  it('dispatches native:change on native:select event from option', () => {
    const el = create();
    const handler = vi.fn();
  ...
  it('updates value on option selection', () => {
    const el = create();
    const opt = el.querySelector('n-option[value="gb"]')!;
    opt.dispatchEvent(new CustomEvent('native:select', {
      bubbles: true,
      composed: true,
  ...
  it('stops internal native:change from listbox (stopImmediatePropagation)', () => {
    const el = create();
...
describe('n-combobox data-driven', () => {
  it('stamps n-input and n-listbox children', () => {
    const el = createDataDriven();
    expect(el.querySelector('n-input')).not.toBeNull();
    expect(el.querySelector('n-listbox')).not.toBeNull();
  });

  it('stamps n-option elements from options attribute', async () => {
    const el = createDataDriven();
    await new Promise(r => queueMicrotask(r));
  ...
  it('dispatches native:change on option selection in data mode', async () => {
    const el = createDataDriven();
    await new Promise(r => queueMicrotask(r));
