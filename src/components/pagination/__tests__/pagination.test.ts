...
function create(attrs: Record<string, string> = {}): HTMLElement {
  const el = document.createElement('n-pagination');
...
describe('n-pagination', () => {
  it('is registered as a custom element', () => {
    expect(customElements.get('n-pagination')).toBeDefined();
  });
  ...
  it('renders prev and next buttons', () => {
    const el = create({ total: '5', value: '3' });
    const prevBtn = el.querySelector('n-button[aria-label="Previous page"]');
    const nextBtn = el.querySelector('n-button[aria-label="Next page"]');
  ...
  it('current page button has aria-current="page"', () => {
    const el = create({ total: '5', value: '3' });
    const currentBtn = el.querySelector('n-button[aria-current="page"]');
  ...
  it('only one page button has aria-current="page"', () => {
    const el = create({ total: '5', value: '2' });
    const currentBtns = el.querySelectorAll('n-button[aria-current="page"]');
    expect(currentBtns).toHaveLength(1);
  });

  it('renders all page buttons for small total (total=5)', () => {
    const el = create({ total: '5', value: '3' });
    const pageBtns = el.querySelectorAll('n-button[aria-label^="Page"]');
  ...
  it('renders no ellipsis for small total', () => {
    const el = create({ total: '5', value: '3' });
    const ellipsis = el.querySelectorAll('.n-pagination-ellipsis');
  ...
  it('renders boundary pages when current is far from boundaries', () => {
    ...
    const btn1 = el.querySelector('n-button[aria-label="Page 1"]');
    const btn20 = el.querySelector('n-button[aria-label="Page 20"]');
    expect(btn1).not.toBeNull();
    expect(btn20).not.toBeNull();
  });

  it('dispatches native:change with correct value when a page button is pressed', () => {
    const el = create({ total: '5', value: '1' });
    const handler = vi.fn();
    el.addEventListener('native:change', handler);

    const page3Btn = el.querySelector('n-button[aria-label="Page 3"]');
    expect(page3Btn).not.toBeNull();
    page3Btn!.dispatchEvent(new Event('native:press', { bubbles: true }));
  ...
  it('updates value property when a page button is pressed', () => {
    const el = create({ total: '5', value: '1' });
    const page4Btn = el.querySelector('n-button[aria-label="Page 4"]');
    page4Btn!.dispatchEvent(new Event('native:press', { bubbles: true }));
  ...
  it('updates value attribute when a page button is pressed', () => {
    const el = create({ total: '5', value: '1' });
    const page2Btn = el.querySelector('n-button[aria-label="Page 2"]');
    page2Btn!.dispatchEvent(new Event('native:press', { bubbles: true }));

    expect(el.getAttribute('value')).toBe('2');
  });

  it('does not dispatch native:change when clicking the current page', () => {
    const el = create({ total: '5', value: '3' });
    const handler = vi.fn();
    el.addEventListener('native:change', handler);

    const currentBtn = el.querySelector('n-button[aria-current="page"]');
    currentBtn!.dispatchEvent(new Event('native:press', { bubbles: true }));
  ...
  it('pressing next button advances to next page', () => {
    ...
    const nextBtn = el.querySelector('n-button[aria-label="Next page"]');
    nextBtn!.dispatchEvent(new Event('native:press', { bubbles: true }));
  ...
  it('pressing prev button goes to previous page', () => {
    ...
    const prevBtn = el.querySelector('n-button[aria-label="Previous page"]');
    prevBtn!.dispatchEvent(new Event('native:press', { bubbles: true }));
  ...
  it('value attribute change updates value signal and re-renders current page indicator', () => {
    const el = create({ total: '5', value: '1' });
    expect(el.querySelector('n-button[aria-current="page"]')?.getAttribute('aria-label')).toBe('Page 1');

    el.setAttribute('value', '4');
    expect((el as any).value).toBe(4);
    expect(el.querySelector('n-button[aria-current="page"]')?.getAttribute('aria-label')).toBe('Page 4');
  });
